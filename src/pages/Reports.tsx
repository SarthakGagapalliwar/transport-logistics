...
import { useReportShipments } from "@/hooks/use-report-shipments";
import {
  Table,
  TableBody,
  TableCell,
...
const Reports = () => {
  ...
  } = useReportShipments(user ?? null);

  const totalRevenue = revenueData.reduce((sum, item) => sum + item.revenue, 0);
  const totalCost = revenueData.reduce((sum, item) => sum + item.cost, 0);
  const totalProfit = totalRevenue - totalCost;

  const totalShipments = shipmentStatusData.reduce(
    (sum, item) => sum + item.count,
    0
  );
  const completedShipments =
    shipmentStatusData.find((s) => s.status === "Completed")?.count || 0;
  const inTransitShipments =
    shipmentStatusData.find((s) => s.status === "In Transit")?.count || 0;
  ...
  const statusData = [
    ...
    {
      name: "Pending",
      value: totalShipments - completedShipments - inTransitShipments,
    },
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      if (user && user.role !== "admin") {
        ...
        if (
          profileData?.assigned_packages &&
          profileData.assigned_packages.length > 0
        ) {
      ...
      const filename = `Shipments_${format(
        dateRange.from,
        "yyyy-MM-dd"
  ...
  const regularUserColumns = [
    ...
    },
    {
      ...
      cell: (row: any) =>
        row.gross_weight !== undefined
          ? `${parseFloat(row.gross_weight).toFixed(2)} tons`
          : "N/A",
    },
    {
      ...
      cell: (row: any) =>
        row.tare_weight !== undefined
          ? `${parseFloat(row.tare_weight).toFixed(2)} tons`
          : "N/A",
    },
    {
      ...
      cell: (row: any) =>
        row.quantity_tons !== undefined
          ? `${parseFloat(row.quantity_tons).toFixed(2)} tons`
          : "N/A",
    },
    {
  ...
  const adminColumns = [
    ...
    },
    {
      ...
      cell: (row: any) =>
        row.routes?.billing_rate_per_ton
          ? `₹${row.routes.billing_rate_per_ton}`
          : "N/A",
    },
    {
      ...
      cell: (row: any) =>
        row.routes?.vendor_rate_per_ton
          ? `₹${row.routes.vendor_rate_per_ton}`
          : "N/A",
    },
    {
    ...
    <DashboardLayout>
      <div className="container mx-auto p-4">
        ...
        <Card>
          ...
          <CardContent>
            {shipmentsError ? (
              <div className="h-60 flex flex-col justify-center items-center text-center gap-2">
                <AlertTriangle className="h-8 w-8 text-destructive" />
                <p className="font-medium text-destructive">
                  Failed to load shipments.
                </p>
                <p className="text-sm text-muted-foreground">
                  {(shipmentsError as Error).message}
                </p>
              </div>
            ) : isShipmentsLoading ? (
                    ...
                    {pagedShipments.length === 0 ? (
                      ...
                      pagedShipments.map((shipment) => (
                        <TableRow key={shipment.id}>
                          ...
                          <TableCell>
                            {shipment.packages?.name || "N/A"}
                          </TableCell>
                          <TableCell>{shipment.source}</TableCell>
                          <TableCell>{shipment.destination}</TableCell>
                          <TableCell>
                            {shipment.materials?.name || "N/A"}
                          </TableCell>
                          {isAdmin && (
                            <TableCell>
                              {shipment.materials?.unit || "N/A"}
                            </TableCell>
                          )}
                          <TableCell>
                            {shipment.transporters?.name || "Unknown"}
                          </TableCell>
                          <TableCell>
                            {shipment.vehicles?.vehicle_number || "Unknown"}
                          </TableCell>
                          <TableCell>
                            {shipment.gross_weight !== undefined &&
                            shipment.gross_weight !== null
                              ? `${parseFloat(shipment.gross_weight).toFixed(
                                  2
                                )} tons`
                              : "N/A"}
                          </TableCell>
                          <TableCell>
                            {shipment.tare_weight !== undefined &&
                            shipment.tare_weight !== null
                              ? `${parseFloat(shipment.tare_weight).toFixed(
                                  2
                                )} tons`
                              : "N/A"}
                          </TableCell>
                          <TableCell>
                            {shipment.quantity_tons !== undefined &&
                            shipment.quantity_tons !== null
                              ? `${parseFloat(shipment.quantity_tons).toFixed(
                                  2
                                )} tons`
                              : "N/A"}
                          </TableCell>
                          <TableCell>
                            {format(parseISO(shipment.departure_time), "PPP")}
                          </TableCell>

                          {isAdmin && (
                            <>
                              <TableCell>
                                {shipment.routes?.billing_rate_per_ton
                                  ? `₹${shipment.routes.billing_rate_per_ton}`
                                  : "N/A"}
                              </TableCell>
                              <TableCell>
                                {shipment.routes?.vendor_rate_per_ton
                                  ? `₹${shipment.routes.vendor_rate_per_ton}`
                                  : "N/A"}
                              </TableCell>
                              <TableCell>
                                {(() => {
                                  const billingRate =
                                    shipment.routes?.billing_rate_per_ton || 0;
                                  const quantity =
                                    parseFloat(shipment.quantity_tons) || 0;
                                  const amount = billingRate * quantity;
                                  return `₹${amount.toFixed(2)}`;
                                })()}
                              </TableCell>
                              <TableCell>
                                {(() => {
                                  const vendorRate =
                                    shipment.routes?.vendor_rate_per_ton || 0;
                                  const quantity =
                                    parseFloat(shipment.quantity_tons) || 0;
                                  const amount = vendorRate * quantity;
                                  return `₹${amount.toFixed(2)}`;
                              ...
                              <TableCell>
                                {(() => {
                                  ...
                                  const quantity =
                                    parseFloat(shipment.quantity_tons) || 0;
                                  const profit =
                                    (billingRate - vendorRate) * quantity;
                                  return `₹${profit.toFixed(2)}`;
                                })()}
                              </TableCell>
                            </>
